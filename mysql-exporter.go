...
func newExporter() *Exporter {
	...
	desc["slowQuery"] = newECSMetric("slowQuery", "slowQuery", []string{"mysql", "sql"})
...
func (e Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	slow := module.GetMysqlSlowQuery()

	for h, l := range slow{
		for k, v := range l{
			newV, err = module.StringToFloat(v)
			if err != nil{
				continue
			}
			ch <- prometheus.MustNewConstMetric(e.newStatusMetric["slowQuery"], prometheus.GaugeValue, newV, h, k)
		}
	}
...
var (
	listenAddress   = flag.String("telemetry.address", ":" + os.Getenv("port"), "Address on which to expose metrics.")
	metricsEndpoint = flag.String("telemetry.endpoint", os.Getenv("endpoint"), "Path under which to expose metrics.")
